...
struct CurrentCityView: View{
    ...
    init(currentCity: City, tabSelection: Binding<Int>, ForecastVM: ForecastViewModel = ForecastViewModel()) {
        self._tabSelection = tabSelection
        self.currentCity = currentCity
        self.ForecastVM = ForecastVM
        
        ForecastVM.weatherData(city: self.currentCity)
    }
                ...
                VStack(spacing: 10){
                    ...
                    VStack{
                        ...
                        HStack{
                            ...
                            Text(getHour(timezone: forecast.timezone, timestamp: forecast.daily.data[0].sunriseTime, identifier: currentCity.countryCode) + "h")
                            Spacer()
                            Image(systemName: "sunset")
                            Text(getHour(timezone: forecast.timezone, timestamp: forecast.daily.data[0].sunsetTime, identifier: currentCity.countryCode) + "h")
                        }
                        Spacer(minLength: 30)
                    }
                    VStack{
                        ...
                        ScrollView(.horizontal, showsIndicators: false){
                            HStack{
                                ...
                                ForEach(forecast.hourly.data[1..<25], id: \.self) { hourly in
                                    VStack(spacing: 10){
                                        Text(getHour(timezone: forecast.timezone, timestamp: hourly.time, identifier: currentCity.countryCode) + "h")
                                        Image(hourly.icon)
                                            .resizable()
                                            .scaledToFit()
                                        if(fahrenheit){
